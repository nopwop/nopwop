...
const wpnopRelatedPosts = (function() {
  ...
  function chosePostsFrom_v1(current, listFromCategories, listFromTags) {
    let ret = []
    let count = 0
    ret = ret.concat(
    ...
    return new Promise(function(resolve, reject){
      ...
      for (let i = 0; i < listOfPost.length; i++) {
        promise = promise.then(media => {
          if (post) callHanlderEach(post, media)
          // for next media
          post = listOfPost[i]
  ...
  function render(options, handler) {
    ...
    function callHanlderEach(post, media) {
      ...
      let p = {
        img: 'img',
        thumbnail: media ? media.media_details.sizes.thumbnail.source_url : defaultThumbnail,
        id: post.id,
        link: post.link,
